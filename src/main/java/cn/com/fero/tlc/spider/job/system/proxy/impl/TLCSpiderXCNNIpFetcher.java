 ...
 */
public class TLCSpiderXCNNIpFetcher extends TLCSpiderIpFetcher {
    private static final String urlPrefix = TLCSpiderPropertiesUtil.getResource("tlc.spider.proxy.url.xcnn");

    @Override
    protected List<String> doFetch() {
        try {
            TLCSpiderLoggerUtil.getLogger().info("开始抓取西刺国内高匿代理");
            ...
            for (TagNode ipNode : ipNodeList) {
                String type = TLCSpiderHTMLParser.parseText(ipNode, "td[7]");
            ...
            TLCSpiderLoggerUtil.getLogger().info("抓取西刺国内高匿代理结束");
        }
    }
}
